...
Result calculateCrossSum(int arr[], int low, int mid, int high) {
    ...
    for (int i = mid; i >= low; i--) {
        if (arr[i] >= 0) {
            ...
            if (currentLeftSum > maxLeftSum || (currentLeftSum == maxLeftSum && (leftLength < leftResult.length || leftResult.length == 0))) {
                maxLeftSum = currentLeftSum;
                leftEndIdx = i;
                leftResult = {mid, leftEndIdx, maxLeftSum, leftLength};
            }
        } else {
    ...
    for (int i = mid + 1; i <= high; i++) {
        if (arr[i] >= 0) {
            ...
            if (currentRightSum > maxRightSum || (currentRightSum == maxRightSum && (rightLength < rightResult.length || rightResult.length == 0))) {
                maxRightSum = currentRightSum;
                rightEndIdx = i;
                rightResult = {mid + 1, rightEndIdx, maxRightSum, rightLength};
            }
        } else {
    ...
    Result finalResult = {leftResult.endIdx, rightResult.endIdx, rightResult.sum + leftResult.sum, rightResult.length + leftResult.length};
    return finalResult;
}
...
Result findMaxValidSubarraySum(int arr[], int low, int high) {
    ...
    if (crossResult.sum == leftResult.sum && crossResult.sum == rightResult.sum) {
        int minLength = min(leftResult.length, min(rightResult.length, crossResult.length));
        if (crossResult.length == minLength) return crossResult;
        if (rightResult.length == minLength) return rightResult;
        if (leftResult.length == minLength) return leftResult;
    }
    if (crossResult.sum == leftResult.sum) {
        int minLength = min(leftResult.length, crossResult.length);
        return (leftResult.length == minLength) ? leftResult : crossResult;
    }
    if (crossResult.sum == rightResult.sum) {
        int minLength = min(rightResult.length, crossResult.length);
        return (rightResult.length == minLength) ? rightResult : crossResult;
    }
